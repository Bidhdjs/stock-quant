...
**说明**：本文件基于当前代码与git改动整理，描述项目现有结构与可用功能，重点突出清晰的分层架构与五大策略特性。

## Scratchpad（当前进度）

- [x] 任务描述：同步git改动到文档，表现分层架构与五大策略
- [x] 计划步骤：1) 更新回测与策略部分 2) 新增五大策略详解 3) 补充Neon与Flask扩展 4) 版本更新
...
- VCPPlus 策略回测：`python -m core.cli backtest --csv ... --strategy VCPPlusStrategy`
  - 示例：`python -m core.cli backtest --csv data/stock/akshare/US.AAPL_AAPL_20211126_20251124.csv --strategy VCPPlusStrategy`
...
### 8.4 VCPPlusStrategy（VCP 强化版）

**特点**：完整 VCP 筛选逻辑落地，保留 Finviz 条件与 RS 评级过滤

**买入原理**（原样保留）：
- Stage 2 趋势模板（MA50/150/200 多头排列 + 52周区间）
- VCP 收缩结构（2-4 次收缩、收缩幅度递减）
- 成交量枯竭（5日均量 < 30日均量）
- 未突破盘整（仍处于收缩区间）
- RS 评级 ≥ 70（后续由 CSV 预留字段提供）

**卖出条件**：
- EMA(5) 跌破触发

**使用场景**：
- ✅ 需要严格 VCP 形态 + RS 过滤的选股回测
- ✅ 与 Finviz 条件结合的观察池筛选
- ❌ 缺少基准/RS 数据时（需在 CSV 中补全）

**参数调优建议**：
- `local_extrema_order`：局部极值灵敏度（默认 10）
- `min_rs_rating`：RS 门槛（默认 70）
- `lookback_period`：VCP 观察窗口（默认 520）

**统计特性**：信号频率 极低 | 交易胜率 高 | 年交易次数 极低

---

### 8.5 VCPStrategyLoose（VCP 宽松版，验证用）
...
| 维度 | Enhanced | Single | VCP | VCPPlus | VCP Loose |
|------|----------|--------|-----|---------|----------|
| **信号频率** | 低 | 中 | 极低 | 极低 | 中 |
| **交易胜率** | 高 | 中 | 很高 | 很高 | 低 |
...
| **技术复杂度** | 中等 | 简单 | 复杂 | 更复杂 | 中等 |
| **参数敏感性** | 中 | 高 | 低 | 低 | 中 |
| **推荐用途** | 稳定收益 | 频繁跟踪 | 趋势捕捉 | 严格选股 | 流程验证 |
...
**记录字段**：
- `date`：信号触发日期
- `signal_type`：信号类型（enhanced_buy/vcp_buy/vcp_plus_buy 等）
